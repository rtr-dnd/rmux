...
final class Workspace: Identifiable, ObservableObject {
    ...
    //
    // Invariants (enforced by `transition(_:reason:)`, see Sources/Async/Workspace+AsyncPhase.swift):
    ...
    //                                           syncStartedAt/plannedDuration == nil
    //
    // All writes must go through `transition(_:reason:)` (convention enforced by
    // review + unit tests). These fields are module-internal so the extension in
    // Sources/Async/Workspace+AsyncPhase.swift can mutate them; do not write to
    // them from elsewhere in the codebase.
