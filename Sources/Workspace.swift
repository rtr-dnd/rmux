...
// MARK: - rmux Async workspace types
// See docs-rmux/spec.md §2 for concepts, §3 for lifecycle. All state transitions
// flow through `Workspace.transition(_:reason:)` (extension at end of file).
...
final class Workspace: Identifiable, ObservableObject {
    ...
    // MARK: - rmux Async workspace state
    //
    // Invariants (enforced by `transition(_:reason:)`, see extension at end of file):
    //   mode == .normal:                        asyncPhase == nil, nextSyncAt == nil,
    //                                           syncStartedAt/plannedDuration == nil
    //   mode == .async + .preparing:            nextSyncAt may carry prior schedule;
    //                                           syncStartedAt/plannedDuration == nil
    //   mode == .async + .syncing:              nextSyncAt == nil,
    //                                           syncStartedAt & plannedDuration set
    //   mode == .async + .selfRunning:          nextSyncAt set (future),
    //                                           syncStartedAt/plannedDuration == nil
    //   mode == .async + .awaitingAttendance:   nextSyncAt set (past),
    //                                           syncStartedAt/plannedDuration == nil
    //
    // All writes go through `transition(_:reason:)`. Do not set these directly.
    @Published private(set) var mode: WorkspaceMode = .normal
    @Published private(set) var asyncPhase: AsyncPhase? = nil
    @Published private(set) var nextSyncAt: Date? = nil
    @Published private(set) var syncStartedAt: Date? = nil
    @Published private(set) var plannedDuration: TimeInterval? = nil
    @Published private(set) var lastSyncEndedAt: Date? = nil
...
// MARK: - rmux Async workspace transitions
//
// All Async phase writes flow through this extension so that invariant validation
// and downstream effects (AgentStateEmitter — Phase 1 Step 6) are centralized.
