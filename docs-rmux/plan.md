...
- **Google Calendar 連携** → Phase 2
- **Self-running の信頼性と discipline**（省電力抑止 / エージェント異常検出 / 自動再起動 /「今すぐ Sync」の強摩擦）→ Phase 3
...
MVP では摩擦は 1 クリック分だけ。フルパス手入力のような強い摩擦は Phase 3（§11）に回す。理由: まずサイクル自体を回して「誤爆の頻度」を実測してから摩擦のレベルを決める。
...
- 戻り値は構造体 `ScheduledSync { at: Date, calendarEventId: String? }`。将来 Phase 2 で `calendarEventId` を足すための余白。
...
- **Google Calendar 連携に備えた型設計**: Phase 1 は `nextSyncAt: Date?` のみ保持する。Phase 2 で `calendarEventId` 等を schema v3 として追加する。§6.7 のスケジュールシートの戻り値は最初から構造体 `ScheduledSync { at: Date, calendarEventId: String? }` にしておき、拡張を容易にする。
...
- **「今すぐ Sync」の誤爆頻度**: MVP は yes/no 1 枚のみ。実測で十分な摩擦がないなら Phase 3 でフルパス手入力を足す（§11）。
...
**ねらい**: Sync session の予定を Google Calendar と双方向に同期する。他の予定（会議・プライベート）と物理的に衝突しなくなり、家族や同僚から見ても通常のスケジュールの一部として扱える。rmux の「いつ Sync するか」の判断材料を、既存のカレンダーという信頼できるソースから引く。

**この phase 後にできるようになること**:
- Sync session を入れると Google Calendar に自動で予定が立つ
- 既に別件が入っている時間帯は候補に出ない（物理衝突ゼロ）
- カレンダー側で予定を動かすと rmux も追従する
- リマインダーは Google Calendar の通知機構に一任できる
- 不在 / 休暇 / 夜間を自動で避けた自然なリズム

**含まれるもの**:
- **統合方式**: **EventKit 採用確定**（2026-04-24）。macOS Calendar.app に追加された Google アカウントを経由して system Calendar を `EKEventStore` で読み書きする。OAuth / Google Cloud Console 不要、同期・オフライン・push は macOS 任せ。詳細は spec §9。
- **予定の create / update / delete**: `endSyncing`（次回予定確定）/ `convertToAsync(.selfRunning)` で create、`reschedule` で move、`revertToNormal` / `endSyncingAndRevert` / `interruptToPreparing` / workspace 閉じ で delete。
- **空き時間ベースのピッカー**: `ScheduleNextSyncSheet` quick-pick を `EKEventStore.events(matching:)` の busy 区間で disable + "Busy" バッジ。Manual DatePicker は自由入力のまま。
- **使用カレンダー選択**: MVP は `defaultCalendarForNewEvents` に書き込み、空き判定は全カレンダー合算。複数カレンダー書き込み先切り替えは後回し。
- **外部変更の取り込み**: delete はフォールバックで新規作成、move は MVP では rmux 側に追従しない（Phase 4 以降で EventKit 変更通知購読を検討）。

**外すもの**:
- Outlook / iCloud Calendar 対応（エコシステムを 1 つに絞る）
- Calendly などの外部予約ツール連携
- MenuBarExtra / 全 workspace のカレンダービュー（Phase 5）

**依存**: Phase 1（MVP の `nextSyncAt` 概念）

**代表的な実装領域**:
- `Sources/Async/CalendarBridge.swift` — `EKEventStore` ラッパ、event CRUD / free/busy 取得 / TCC 許可プロンプト
- `Sources/Async/ScheduleNextSyncSheet.swift` の拡張（busy バッジ + disable）
- 永続化 schema v3 で `SessionWorkspaceSnapshot.calendarEventId: String?` 追加
- `Workspace.calendarEventId: String?` + `Workspace+AsyncPhase.swift` の transition に event lifecycle を配線
- `Resources/Info.plist` に `NSCalendarsFullAccessUsageDescription` / `NSCalendarsUsageDescription`
- （後日）設定画面に「Calendar 連携」セクション（権限状態表示・書き込み先カレンダー選択）

**懸念・未解決**（Phase 2 中に扱う / 次フェーズ送り）:
1. **外部カレンダーの move 追従**: MVP は rmux → Calendar の一方向同期のみ。`EKEventStoreChanged` 通知を購読して rmux の `nextSyncAt` に反映する経路は Phase 4 以降。
2. **`calendarEventId` の失効検出**: 外部で delete → recreate されると別 id になる。MVP は update 失敗で create フォールバック。孤立 event の掃除は orphan sweep（Phase 4）で扱う。
3. **複数デバイスで同じアカウントを使ったときの同期の一貫性**: rmux A が作った event を rmux B が別 workspace に紐付けないように。Event `notes` の `rmux://workspace/<UUID>` マーカーを識別子にして将来リカバリ可能にしてあるが、現状は読まない。
4. **設定画面の配置**: cmux 既存 Settings pane に「Async」セクションを新設するか、既存に寄生するか。Phase 2 終盤で検討。
5. **書き込み先カレンダーの切り替え**: MVP は `defaultCalendarForNewEvents` 固定。rmux 専用サブカレンダー指定は設定 UI 整備後。

---

### Phase 3: Self-running の信頼性と discipline
...
**依存**: Phase 1。Phase 3 の liveness 監視があると稼働率の精度が上がる。
...
- **カレンダービュー**: Async の Sync を週 / 日単位で俯瞰。Google Calendar のイベントと同じ表示 (Phase 2)。ドラッグで時間変更 → Calendar API に伝播。
- **プロジェクトメタデータ**: 目的・期限・関係者・ゴール条件などを Workspace に付与し、ダッシュボードに表示。
- **運営メトリクス**: 週次 / 月次で「async に割いた総時間」「実稼働比率」などをまとめて見せる。
- **cmux 内の Sync session 衝突チェック**: `ScheduleNextSyncSheet` でスケジュール選択時、既存 Async workspace の `nextSyncAt` と ±30 分以内の候補を disable。Google Calendar 連携時は Phase 2 の free/busy と統合される。

**外すもの**:
- リモート / 他デバイスでの同期（「チームで同じプロジェクトを見る」は別の次元の話）
- AI を使った「このプロジェクトは詰まり気味です」のような自動アラート（過度な能動化は「脳に優しい」思想に反する）

**依存**: Phase 1 / 2 / 4（俯瞰に値するだけの情報が Phase 4 まででやっと揃う）
